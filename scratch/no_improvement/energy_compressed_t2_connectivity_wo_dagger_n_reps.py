...
from lucj.tasks.lucj_compressed_t2_connectivity_task import LUCJCompressedT2ConnectivityTask
from lucj.tasks.lucj_compressed_t2_connectivity_wo_dagger_task import LUCJCompressedT2ConnectivityWoDaggerTask
...
tasks_compressed_t2_connectivity_wo_dagger = [
...
for task in tasks_compressed_t2_gradient:
    filepath = DATA_ROOT / "lucj_compressed_t2_gradient" / task.dirpath / "data.pickle"
    with open(filepath, "rb") as f:
        result = pickle.load(f)
        results_compressed_t2_gradient[task] = result

results_compressed_t2_connectivity = {}
for task in tasks_compressed_t2_connectivity:
    filepath = DATA_ROOT / "lucj_compressed_t2_connectivity" / task.dirpath / "data.pickle"
    with open(filepath, "rb") as f:
        result = pickle.load(f)
        results_compressed_t2_connectivity[task] = result

results_compressed_t2_connectivity_wo_dagger = {}
for task in tasks_compressed_t2_connectivity_wo_dagger:
    filepath = DATA_ROOT / "lucj_compressed_t2_connectivity_wo_dagger" / task.dirpath / "data.pickle"
    with open(filepath, "rb") as f:
        result = pickle.load(f)
        results_compressed_t2_connectivity_wo_dagger[task] = result
...
for i, connectivity in enumerate(connectivities):
    ...
    energies = [results_compressed_t2_connectivity_wo_dagger[task]['energy'] for task in tasks_compressed_t2_connectivity_wo_dagger]
    errors = [results_compressed_t2_connectivity_wo_dagger[task]["error"] for task in tasks_compressed_t2_connectivity_wo_dagger]
    init_loss = [results_compressed_t2_connectivity_wo_dagger[task]["init_loss"] for task in tasks_compressed_t2_connectivity_wo_dagger]
    final_loss = [results_compressed_t2_connectivity_wo_dagger[task]["final_loss"] for task in tasks_compressed_t2_connectivity_wo_dagger]

    axes[0, i].plot(
        these_n_reps,
        errors,
        f"{markers[0]}{linestyles[0]}",
        label="LUCJ Compressed-t2 connectivity-wod",
        color=colors[7],
    )
    axes[1, i].plot(
        these_n_reps,
    ...
    leg = axes[0, 0].legend(bbox_to_anchor=(1, -1.6), loc="upper center", ncol = 3)
