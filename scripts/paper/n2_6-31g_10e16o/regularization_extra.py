...
for i, bond_distance in enumerate(bond_distances):
    ...
    vqe_error_lucj_full = results["error"]
    ...
    tasks_compressed_t2_reg_no_multi = [
        SQDEnergyTask(
            molecule_basename=molecule_basename,
            bond_distance=bond_distance,
            lucj_params=LUCJParams(
    ...
    for task in tasks_compressed_t2_reg:
        filepath = DATA_ROOT / task.operatorpath / "opt_data.pickle"
        results = load_data(filepath)
        list_loss[2].append(results["final_loss"])
    for task in tasks_compressed_t2_reg_no_multi:
        filepath = DATA_ROOT / task.operatorpath / "opt_data.pickle"
        results = load_data(filepath)
    ...
    labels = [
        "truncated",
        "compressed",
        r"compressed with regularization $\lambda = 5 \times 10^{-3}$",
        r"compressed with regularization $\lambda = 5 \times 10^{-3}$ (no multi-stage)",
    ]
