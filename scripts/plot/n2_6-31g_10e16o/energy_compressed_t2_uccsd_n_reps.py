...
from lucj.tasks.uccsd_compressed_t2_params_task import UCCSDCompressedT2ParamsTask
...
results_uccsd_compressed_t2 = {}
for task in tasks_uccsd_compressed_t2:
    filepath = DATA_ROOT / "uccsd_compressed_t2_params" / task.dirpath / "data.pickle"
    with open(filepath, "rb") as f:
        results_uccsd_compressed_t2[task] = pickle.load(f)
...
for i, connectivity in enumerate(connectivities):
    ...
    #     print(results_compressed_t2[task])
    #     input()
    energies = [results_uccsd_compressed_t2[task]['energy'] for task in tasks_uccsd_compressed_t2]
    errors = [results_uccsd_compressed_t2[task]["error"] for task in tasks_uccsd_compressed_t2]
    # final_loss = [results_uccsd_compressed_t2[task]["final_loss"] for task in tasks_uccsd_compressed_t2]

    axes[0, i].plot(
        these_n_reps,
        errors,
        f"{markers[1]}{linestyles[0]}",
        label="UCCSD Compressed-t2",
        color=colors[5],
    )
    axes[1, i].plot(
        [],
