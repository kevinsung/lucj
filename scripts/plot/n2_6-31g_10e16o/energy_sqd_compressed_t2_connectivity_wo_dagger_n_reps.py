...
from lucj.tasks.lucj_sqd_compressed_t2_connectivity_wo_dagger_task import LUCJSQDCompressedT2ConnectivityWoDaggerTask
...
tasks_compressed_t2_connectivity_wo_dagger = [
    LUCJSQDCompressedT2ConnectivityWoDaggerTask(
        molecule_basename=molecule_basename,
        bond_distance=bond_distance,
        lucj_params=LUCJParams(
            connectivity=connectivity,
...
for task in tasks_compressed_t2_connectivity_wo_dagger:
    filepath = DATA_ROOT / "lucj_sqd_compressed_t2_connectivity_wo_dagger" / task.dirpath / "data.pickle"
    with open(filepath, "rb") as f:
        result = pickle.load(f)
...
for samples_per_batch in samples_per_batch_range:
    ...
    for i, connectivity in enumerate(connectivities):
        ...
        sci_vec_shape = [results_compressed_t2_connectivity_wo_dagger[task]["sci_vec_shape"][0] * results_compressed_t2_connectivity_wo_dagger[task]["sci_vec_shape"][0] for task in tasks_compressed_t2_connectivity_wo_dagger]
