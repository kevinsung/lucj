...
for samples_per_batch, connectivity, bond_distance in itertools.product(
    ...
    for i, max_dim in enumerate(max_dim_range):
        ...
        axes[row_sci_vec_dim, i].axhline(
            ...
            * data_lucj[task_lucj]["sci_vec_shape"][1],
            linestyle="--",
            label=f"LUCJ full ({full_n_reps} reps)",
            color=colors[1],
        )
        ...
        # init_loss = [
        #     results_compressed_t2[task]["init_loss"] for task in tasks_compressed_t2
        # ]
        # final_loss = [
        #     results_compressed_t2[task]["final_loss"] for task in tasks_compressed_t2
        # ]
        # print(errors)
        sci_vec_shape = [
        ...
        if connectivity != "all-to-all":
            ...
            for task in tasks_compressed_t2_connectivity:
                filepath = (
                    DATA_ROOT / task.dirpath / "sqd_data.pickle"
                )
