...
for i, connectivity in enumerate(connectivities):
    ...
    # energies = [data[task]["energy"] for task in tasks_uccsd]
    # errors = [data[task]["error"] for task in tasks_uccsd]
    # spin_squares = [data[task]["spin_squared"] for task in tasks_uccsd]
    # axes[0, i].plot(
    #     bond_distance_range,
    #     energies,
