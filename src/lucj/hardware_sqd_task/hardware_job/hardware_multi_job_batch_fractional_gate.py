...
def run_on_hardware(
    ...
    list_jobs = []
    for _ in range(len(list_sample_filenames)):
        job = sampler.run(list_isa_circuit, shots=shots)
        list_jobs.append(job)

    meas_circuit = []

    for job, sample_filenames in zip(list_jobs, list_sample_filenames):
        primitive_result = job.result()
        meas_circuit.append([])
        for pub_result, sample_filename in zip(primitive_result, sample_filenames):
            meas_circuit[-1].append(pub_result.data.meas)
