...
hardware_path = "dynamic_decoupling_xy_opt_0_fractional_gate_0828"
...
def load_operator(task: HardwareSQDEnergyTask, data_dir: str, mol_data):
    ...
    random_operator = ffsim.random.random_ucj_op_spin_balanced(
        norb,
    ...
    compressed_operator = ffsim.UCJOpSpinBalanced(
...
def run_hardware_sqd_energy_batch_task(
    random_task: HardwareSQDEnergyTask,
    truncated_task: HardwareSQDEnergyTask,
    compressed_task: HardwareSQDEnergyTask,
    *,
    data_dir: Path,
    ...
    if random_task.molecule_basename == "fe2s2_30e20o":
        mol_data = load_molecular_data(
            random_task.molecule_basename,
            molecules_catalog_dir=molecules_catalog_dir,
        )
    else:
        mol_data = load_molecular_data(
            f"{random_task.molecule_basename}_d-{random_task.bond_distance:.5f}",
            molecules_catalog_dir=molecules_catalog_dir,
        )
    norb = mol_data.norb
    nelec = mol_data.nelec
