...
def run_lucj_sqd_quimb_task(
    ...
    *,
    ...
    def fun(x: np.ndarray) -> float:
        ...
        # vacuum state
        peps = quimb.tensor.TN_from_sites_product_state({q: [1.0, 0.0] for q in circuit.num_qubits})
        ...
        for gate in tqdm.tqdm(quimb_gates(decomposed)):
            peps.gate_simple_(
                gate.array,
                gate.qubits,
                gauges=gauges,
        ...
        samples = peps.sample(task.shots, seed=task.seed)
