...
class LUCJSQDTask:
    ...
    params: str  # circuit params, i.e. "ccsd" or "random"
    params_seed: int | None  # used if params is "random"
    shots: int
    ...
    def dirpath(self) -> Path:
        return (
            ...
            / f"params-seed-{self.params_seed}"
            / f"shots-{self.shots}"
...
def run_lucj_sqd_task(
    ...
    *,
    ...
    sci_state_filename = data_dir / task.dirpath / "sci_state.npz"
    if (
        (not overwrite)
        and os.path.exists(data_filename)
        and os.path.exists(sci_state_filename)
    ):
    ...
    for i, batch in enumerate(batches):
        energy_sci, sci_state, _, spin = solve_fermion(
            batch,
    ...
    with open(data_filename, "wb") as f:
        pickle.dump(data, f)
    sci_state.save(sci_state_filename)
